...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      .from('events')
      .select('id, name, description, start_date, end_date, lat, lng, city, country, event_type, tier, venue_name, venue_address, event_time, registration_url, is_side_event, parent_event_id, tags, status')
      .eq('status', 'confirmed')
      .not('lat', 'is', null)
      .not('lng', 'is', null)
